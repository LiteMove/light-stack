...
type User struct {
	...
	// 关联关系
	Roles  []systemModel.Role  `json:"roles,omitempty" gorm:"many2many:user_roles;"`
	Tenant *systemModel.Tenant `json:"tenant,omitempty" gorm:"foreignKey:TenantID"`
}
...
type UserProfile struct {
	...
	Roles       []systemModel.RoleProfile  `json:"roles,omitempty"`
	RoleCodes   []string                   `json:"roleCodes,omitempty"`
	Menus       []systemModel.MenuTreeNode `json:"menus,omitempty"`
...
func (u *User) ToProfile() UserProfile {
	...
	// 转换角色信息
	if len(u.Roles) > 0 {
		roles := make([]systemModel.RoleProfile, 0, len(u.Roles))
